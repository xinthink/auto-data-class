 ...
 */
internal class GsonTypeAdapterGenerator : FacetGenerator {
    ...
    private fun FunSpec.Builder.generatePropertyReader(
        paramJsonReader: String,
        propDef: DataPropDef
    ) {
    ...
    private fun dumpParameterizedTypeToken(
        propType: ParameterizedTypeName,
        asType: Boolean = false
    ) = CodeBlock.builder()
        .addParameterizedType(propType, asType)
        .build()

    private fun CodeBlock.Builder.addParameterizedType(
        propType: ParameterizedTypeName,
        asType: Boolean = false // produce a `Type` instead of a `TypeToken`
    ): CodeBlock.Builder =
